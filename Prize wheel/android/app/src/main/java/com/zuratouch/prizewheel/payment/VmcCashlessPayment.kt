...
class VmcCashlessPayment(private val session: VmcSession) : PaymentTerminal {
    ...
    override suspend fun refundPayment(amountCents: Long, lane: Int): Boolean {
        ...
        if (tryCashlessRefund()) return true
        return tryCoinRefund()
    }

    private suspend fun tryCashlessRefund(): Boolean =
        when (val response = session.send(VmcProtocol.cancelCashless())) {
            is VmcResponse.Acknowledged -> response.command == VmcProtocol.Command.CANCEL_CASHLESS
            else -> false
        }

    private suspend fun tryCoinRefund(): Boolean =
        when (val response = session.send(VmcProtocol.refundCoins())) {
