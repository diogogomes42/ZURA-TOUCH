...
class VmcVendingMachineTest {
    private val spiral = Spiral("A1", 1, 1, 3, 10, listOf(com.zuratouch.prizewheel.domain.QueuedProduct("drink", "Água")))
    ...
    @Test fun `delivers when machine confirms completion`() = runTest {
        ...
        assertEquals(DispenseResult.Delivered, machine.dispense(spiral))
    ...
    @Test fun `fails when delivery status is not completed`() = runTest {
        ...
        val result = machine.dispense(spiral)
